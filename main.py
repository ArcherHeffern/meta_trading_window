...
def process_trading_window(cur_plot: Any, start_of_month: datetime, group_by_month: defaultdict[datetime, list[tuple[datetime, float]]]) -> TradingWindowMetrics:
    cur_plot.set_title(f"{start_of_month.day}/{start_of_month.month}/{start_of_month.year}")
...
def get_data()-> tuple[defaultdict[datetime, list[tuple[datetime, float]]], int]:
    group_by_month: defaultdict[datetime, list[tuple[datetime, float]]] = defaultdict(list)
...
def setup_plot(num_trading_windows: int) -> tuple[Figure, Any]:
    ...
    if (num_trading_windows / NUM_SUBPLOT_COLUMNS) != num_trading_windows // NUM_SUBPLOT_COLUMNS:
        num_subplot_rows += 1
...
def main():
    ...
    for start_of_month in group_by_month.keys(): 
        ...
        trading_window_metrics[start_of_month] = process_trading_window(cur_plot, start_of_month, group_by_month)
        trading_windows_processed += 1

    # Meta metrics computation
    avg_avg_zscore = sum(m.avg_zscore for m in trading_window_metrics.values()) / len(trading_window_metrics)
    avg_avg_extra_money = sum(m.avg_extra_money for m in trading_window_metrics.values()) / len(trading_window_metrics)
